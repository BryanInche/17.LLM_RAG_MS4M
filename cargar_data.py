...
def crear_vectorstore(docs):
    ...
    try:
        vector_store = Chroma.from_documents(
            ...
            embedding=embeding_modelo,
            persist_directory="chroma_ms4m_basedatos",
            collection_name="data_ms4m_despacho" #colection dentro del vs
        )
...
## Validando el funcionamiento de las funciones
...
try:
    documentos_all = cargar_documentos(rutas_archivos)
    print(f"Se cargaron {len(documentos_all)} fragmentos de texto.")
except Exception as e:
...
try:
    vector_store = crear_vectorstore(documentos_all)
    print("Vector store creado y validado correctamente.")
except Exception as e:
    print(f"Error al crear el vector store: {e}")
'''
